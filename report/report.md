...
**Date:** May 2026  
**Repository:** [github.com/Stokes7/helmholtz-muffler-fem](https://github.com/Stokes7/helmholtz-muffler-fem)  
...
The project is organized into four directories (`gmsh/`, `src/`, `scripts/`, `test/`), keeping geometry, solver, parametric studies, and tests fully decoupled. Meshes are generated with the Gmsh Python API using the OCC kernel; boolean operations build both geometries from a single function call, and boundary tags (1: inlet, 2: outlet, 3: walls) are assigned in Gmsh and passed directly to DOLFINx. The `HelmholtzSolver` class assembles the weak form once at construction time and exposes $\omega$ and $k$ as `fem.Constant` objects, so a frequency sweep updates only two scalars without recompiling the UFL forms. The linear system is solved with MUMPS via PETSc, which is robust and fast for the complex-valued indefinite systems at the mesh sizes used here. Three lightweight pytest tests check mesh validity, the protrusion collision guard, and a physically plausible TL bound. The Conda environment is fully pinned in `environment.yml`, and a GitHub Actions pipeline builds and deploys this report on every push to `main`.
